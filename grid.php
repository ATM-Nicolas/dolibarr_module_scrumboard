...
function _draw_grid(&$TWorkstation, $column_width) {
	...
	foreach($TWorkstation as $w_id=>$w_param) {
		...
		?><div id="columm-ws-<?php echo $w_id; ?>" valign="top" style="float:left;margin-right: 5px; width:<?php echo round($w_column); ?>px; <?php echo $back; ?> border:1px solid #666;">
		        <div style="width:<?php echo $column_width ?>px; overflow: hidden;"><span class="fixedHeader columnHeader"><a href="javascript:toggleWorkStation(<?php echo $w_id; ?>)" ws-id="<?php echo $w_id; ?>"><?php echo $w_param['name'].($w_param['velocity']!=1 ? ' '.round($w_param['velocity']*100).'%' : ''); ?></a></span></div>
